    ...
    <div class="grid grid-cols-2 gap-12 mb-10">
        ...
        <section>
            ...
            <div class="space-y-2">
                ...
                <p class="text-xs text-slate-600">{{ fetch_data($loan?->borrower?->phone ?? null) }}</p>
                <p class="text-xs text-slate-600 font-mono">BVN: {{ fetch_data($loan?->borrower?->bvn ?? 'N/A' ?? null) }}</p>
            </div>
        </section>
        ...
        <section>
            ...
            <div class="space-y-2">
                ...
                <p class="text-xs text-slate-600">Principal: ₦{{ fetch_data($loan?->amount?->format() ?? null) }}</p>
                @php 
                ...
                @endphp
                <p class="text-xs text-slate-600 font-bold">Balance: <span class="text-primary">₦{{ fetch_data($balance?->format() ?? null) }}</span></p>
            </div>
        </section>
    </div>
        ...
        <table class="w-full text-left border-collapse border border-slate-200">
            ...
            <tbody class="divide-y divide-slate-200">
                @forelse($loan->repayments as $repayment)
                    <tr class="text-xs">
                        <td class="px-4 py-3 border-r border-slate-200 font-medium">{{ fetch_data($repayment?->paid_at?->format('M d, Y') ?? null) }}</td>
                        <td class="px-4 py-3 border-r border-slate-200 uppercase font-bold text-slate-500">{{ fetch_data($repayment?->payment_method ?? null) }}</td>
                        <td class="px-4 py-3 border-r border-slate-200">₦{{ fetch_data($repayment?->principal_amount?->format() ?? null) }}</td>
                        <td class="px-4 py-3 border-r border-slate-200">₦{{ fetch_data($repayment?->interest_amount?->format() ?? null) }}</td>
                        <td class="px-4 py-3 text-right font-black text-slate-900">₦{{ fetch_data($repayment?->amount?->format() ?? null) }}</td>
                    </tr>
                @empty
                    <tr>
            ...
            <tfoot>
                <tr class="bg-slate-900 text-white font-black">
                    ...
                    <td class="px-4 py-3 text-right">₦{{ fetch_data($totalPaid?->format() ?? null) }}</td>
                </tr>
            </tfoot>
        </table>
    </section>
