<div class="max-w-7xl mx-auto w-full">
    <div class="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
            ...
            <p class="text-gray-500 mt-1">Configure and extend the questionnaire for your customers.</p>
        </div>
        
        <div class="flex bg-slate-100 dark:bg-zinc-800 p-1 rounded-xl">
            @foreach($formTypes as $type => $label)
                <button wire:click="$set('currentFormType', '{{ $type }}')" class="px-6 py-2.5 rounded-lg text-xs font-black uppercase tracking-widest transition-all {{ $currentFormType === $type ? 'bg-white dark:bg-zinc-700 shadow-sm text-primary dark:text-white' : 'text-slate-400 hover:text-slate-600' }}">
                    {{ $label }}
                </button>
            @endforeach
        </div>
